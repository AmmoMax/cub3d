...
int main()
{
	...
	config_p = &config;
	valid = "S ./path_to_the_sprite_texture    ";
	invalid = "ASS ./path_to_the_sprite_texture";
	// invalid = "SS ./path_to_the_sprite_texture";
	if (sprite_handler(valid, &config_p) == 0)
	{
		printf("Line with sprite is valid!\n");
		printf("Line parsed!\n");
		printf("Sprite path = %s", config.s_texture);
	}
	else
		printf("Line with sprite is NOT valid!\n");
	return (0);
